...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (numPoints > 0) {
            this.f139mX = new float[numPoints];
            this.f140mY = new float[numPoints];
            float[] position = new float[2];
            for (int i = 0; i < numPoints; i++) {
                pathMeasure.getPosTan((((float) i) * pathLength) / ((float) (numPoints - 1)), position, (float[]) null);
                this.f139mX[i] = position[0];
                this.f140mY[i] = position[1];
            }
            if (((double) Math.abs(this.f139mX[0])) > 1.0E-5d || ((double) Math.abs(this.f140mY[0])) > 1.0E-5d || ((double) Math.abs(this.f139mX[numPoints - 1] - 1.0f)) > 1.0E-5d || ((double) Math.abs(this.f140mY[numPoints - 1] - 1.0f)) > 1.0E-5d) {
                throw new IllegalArgumentException("The Path must start at (0,0) and end at (1,1) start: " + this.f139mX[0] + "," + this.f140mY[0] + " end:" + this.f139mX[numPoints - 1] + "," + this.f140mY[numPoints - 1]);
            }
            float prevX = 0.0f;
    ...
    public float getInterpolation(float t) {
        ...
        int startIndex = 0;
        int endIndex = this.f139mX.length - 1;
        while (endIndex - startIndex > 1) {
            int midIndex = (startIndex + endIndex) / 2;
            if (t < this.f139mX[midIndex]) {
                endIndex = midIndex;
            } else {
                startIndex = midIndex;
            }
